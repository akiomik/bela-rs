...
pub(crate) fn generate(root: &Path, sysroot: Option<PathBuf>) {
    ...
    let mut builder = bindgen::Builder::default()
        ...
        // to reach a pin that is not one of the sixteen digital
        // channels a `BelaContext` carries, or to reach any pin at all
        // outside a render callback. `PIN_DIRECTION` and `PIN_VALUE`
        // are the vocabulary its `out_flag` and `value` arguments are
        // written in, so they come with the functions.
