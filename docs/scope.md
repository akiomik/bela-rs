...
| `gpio_setup`, `gpio_export`, `gpio_unexport`, `gpio_set_dir`, `gpio_set_value`, `gpio_get_value`, `gpio_set_edge`, `gpio_fd_open`, `gpio_fd_close`, `gpio_write`, `gpio_read`, `gpio_dismiss`, `led_set_trigger` | The whole of `GPIOcontrol.h`: sysfs GPIO, one pin at a time. It is the only path *this crate offers* to a pin outside a render callback, and the only one it offers at all to a pin that is not among the sixteen digital channels; `led_set_trigger` has no other path anywhere. libbela reaches a pin a second way, through the registers, which is the `Gpio` bullet under [Not written yet](#not-written-yet) and is not this. What the safe wrapper waits on is [#156](https://github.com/akiomik/bela-rs/issues/156), and it is measurement rather than design. These are `open`, `read` and `write` on `/sys/class/gpio`, so they must not be reachable from `render`, which an API has to make hard rather than warn about — the problem MIDI output had, and which [midi.md](midi.md) answered with an auxiliary task rather than a note. libbela claims `gpio584`, `gpio585` and `gpio586` through this same interface, for the two LEDs and the stop button ([board-facts.md](board-facts.md)), and half of what an application asking for one of those gets is already settled by reading `core/GPIOcontrol.cpp:75-82`: `gpio_export` opens `/sys/class/gpio/gpioN` first and returns `0` if it is there, so exporting a pin libbela already exported succeeds, indistinguishably from a fresh export. The guard on that is `if(fd > 0)` rather than `>= 0`, so a process that has closed its standard input can be handed descriptor 0 by that `open`, miss the fast path, leak the descriptor and then fail the real export with `EBUSY` — which is the one way an already-exported pin comes back as a failure, and a wrapper built on this has to survive it. Nothing refuses; the program simply gets the direction and value files of libbela's pin, and a later `gpio_dismiss` unexports a pin libbela still holds. What is left to measure is what that does to the run, which needs a board with audio up. The same page measured that the stop button's export outlives a run where the LEDs' does not, so whichever RAII shape a wrapper takes will encode a choice between them. And nothing yet maps a GPIO number to a Gem header pin, so there is nothing to check an argument against. `led_set_trigger` adds one more, and this one is now measured: it builds `/sys/class/leds/beaglebone:green:usr%d/trigger` (`core/GPIOcontrol.cpp:332`, the `#else` of an `#ifdef IS_AM62_SK` that a Gem build does not take), and a Gem has `usr1` through `usr4` where the BeagleBone that path is written for has `usr0` through `usr3` — so a `lednum` of 0 names no file at all ([board-facts.md](board-facts.md)). |
